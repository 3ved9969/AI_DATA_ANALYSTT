...
def get_storage_path(dataset_id: str) -> str:
    return os.path.join(app.config["UPLOAD_FOLDER"], f"{dataset_id}.parquet")
...
def upload_file():
    try:
        ...
        # Secure Parquet Storage Matrix Allocation
        dataset_id = uuid.uuid4().hex
        df.to_parquet(get_storage_path(dataset_id), index=False, compression="snappy")
...
def query_data():
    try:
        ...
        if not os.path.exists(storage_path):
            return jsonify({"error": "Dataset cache missing or expired"}), 400

        if not question:
            return jsonify({"error": "Question is required"}), 400

        # High-Speed Streaming on Demand from Disk Snapshot
        ...
        # Coordinate alignment handshake passed directly to chart generator
